...
11. 使用配置文件进行收集箱管理(可选)
    ...
    - 参考如下示例进行收集箱配置
      ...
      - `enable` 字段的值为该群是否开启收集功能, 默认为 `true`
      - `reply` 字段的值为处理完发到收集箱中的消息后是否将处理结果回复到收集箱中, 默认为 `true`
...
12. 使用超级用户账户向机器人账户发送如下命令进行收集箱管理(可选)
    ...
    - `从收集箱移除 *[群号]`
      ...
      - 注2: 移除的群可以再次设置为收集箱
    - `开启收集 *[群号]`
      - 示例: `开启收集 123456789 987654321`
      - `*[群号]` 即为想要开启收集功能的、作为收集箱的群号列表, 多个群号中间使用空格隔开
      - 注1: 添加收集箱后默认开启收集
    - `关闭收集 *[群号]`
      - 示例: `关闭收集 123456789 987654321`
      - `*[群号]` 即为想要关闭收集功能的、作为收集箱的群号列表, 多个群号中间使用空格隔开
    - `开启消息回复 *[群号]`
      - 示例: `开启消息回复 123456789 987654321`
      - `*[群号]` 即为想要开启消息回复的、作为收集箱的群号列表, 多个群号中间使用空格隔开
      - 注1: 某个收集箱开启消息回复后 bot 将会在收集箱中每条消息处理完成后发送处理结果至该收集箱中
      - 注2: 添加收集箱后默认开启消息回复
    - `关闭消息回复 *[群号]`
      - 示例: `关闭消息回复 123456789 987654321`
      - `*[群号]` 即为想要关闭消息回复的、作为收集箱的群号列表, 多个群号中间使用空格隔开
      - 注1: 某个收集箱关闭消息回复后 bot 将不会向收集箱群中发送收集箱内容的处理结果, 消息处理错误信息将通过私聊发送给超级用户
      - 注2: 可以用于偷偷接收并备份某个群的消息哦~
    - `列出收集箱`
      - 示例: `列出收集箱`
      - 该命令会列出所有的作为收集箱的群号
